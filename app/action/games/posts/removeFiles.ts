...
export async function removeFiles(method: HttpMethod, post_id: number, file_id: number): Promise<ApiResponse> {
    ...
    try {
        switch (method) {
            case 'DELETE': {
                const res = await fetch(`${process.env.API_URL}/games/posts/${post_id}/files/${file_id}`, {
                    method: method,
                    headers: {
